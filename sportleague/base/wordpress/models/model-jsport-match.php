...
class modelJsportMatch
{
    ...
    public function getPlayerEvents()
    {
        ...
        if($season_id){
            ...
            $mplayed = get_post_meta( $this->match_id, '_joomsport_match_played', true );
            
            
            $sObj = new modelJsportSeason($season_id);
            $single = $sObj->getSingle();
            $this->lists['m_events_home'] = $this->lists['m_events_away'] = array();
            $query = 'SELECT me.*,ev.*'
                            .' FROM '.DB_TBL_MATCH_EVENTS.' as me '
                            .' JOIN '.DB_TBL_EVENTS.' as ev ON me.e_id = ev.id'

                            ." WHERE me.match_id = ".$this->match_id." AND  ev.player_event = '1'"
            ...
            //if(count($this->lists['m_events_all'])){
                $query = 'SELECT me.*,ev.*'
                                .' FROM '.DB_TBL_MATCH_EVENTS.' as me '
                                .' JOIN '.DB_TBL_EVENTS.' as ev ON me.e_id = ev.id'

                                ." WHERE me.match_id = ".$this->match_id." AND ev.player_event = '1'"
                                .' AND '.($single ? 'me.player_id='.intval($home_team) : 'me.t_id='.intval($home_team))
                                .' ORDER BY CAST(me.minutes as DECIMAL(10,5)),me.eordering';
                ...
                $query = 'SELECT me.*,ev.*'
                                ...
                                ." WHERE me.match_id = ".$this->match_id." AND ev.player_event = '1'"
                                .' AND '.($single ? 'me.player_id='.intval($away_team) : 'me.t_id='.intval($away_team))
                                .' ORDER BY CAST(me.minutes as DECIMAL(10,5)),me.eordering';
    ...
    public function getLineUps()
    {
        ...
        $query = "SELECT s.* "
                ...
                . " WHERE match_id={$this->match_id} AND team_id={$home_team} AND squad_type = 1"
