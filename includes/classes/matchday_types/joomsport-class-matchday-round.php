...
class JoomSportClassMatchdayRound{
    ...
    public function getViewEdit(){
		...
		<tbody>
			...
			<th scope="row"><label for="name"><?php echo __("Playoff","joomsport-sports-league-results-management");?></label></th>
			<td>
    ...
    public function save(){
        ...
        if(isset($fdata['match_id']) && count($fdata['match_id'])){
            for($intA = 0; $intA < count($fdata['match_id']); $intA++){
                ...
                if(intval($fdata['match_id'][$intA])){
                    ...
                    $postM = get_post(intval($fdata['match_id'][$intA]));
                ...
                if($post_id){
                    ...
                    if(!isset($allmeta["_joomsport_home_team"][0]) || $allmeta["_joomsport_home_team"][0] != intval($fdata['home_team'][$intA])){
                        update_post_meta($post_id, '_joomsport_home_team', intval($fdata['home_team'][$intA]));
                    }
                    if(!isset($allmeta["_joomsport_away_team"][0]) || $allmeta["_joomsport_away_team"][0] != intval($fdata['away_team'][$intA])){
                    
                        update_post_meta($post_id, '_joomsport_away_team', intval($fdata['away_team'][$intA]));
                    }
                    if(!isset($allmeta["_joomsport_home_score"][0]) ||  $allmeta["_joomsport_home_score"][0] != intval($fdata['home_score'][$intA]) || ($allmeta["_joomsport_home_score"][0] == '' && ($fdata['home_score'][$intA]) != '')){
                        update_post_meta($post_id, '_joomsport_home_score', intval($fdata['home_score'][$intA]));
                        $scoreChanged = true;
                    }
                    if(!isset($allmeta["_joomsport_away_score"][0]) || $allmeta["_joomsport_away_score"][0] != intval($fdata['away_score'][$intA]) || ($allmeta["_joomsport_away_score"][0] == '' && ($fdata['away_score'][$intA]) != '')){
                    ...
                    if(isset($fdata['m_played'][$intA]) && (!isset($allmeta["_joomsport_match_played"][0]) || $allmeta["_joomsport_match_played"][0] != intval($fdata['m_played'][$intA]))){
                        update_post_meta($post_id, '_joomsport_match_played', intval($fdata['m_played'][$intA])); 
                    }
                    if(isset($fdata['m_date'][$intA]) && (!isset($allmeta["_joomsport_match_date"][0]) || $allmeta["_joomsport_match_date"][0] != ($fdata['m_date'][$intA]))){
                        update_post_meta($post_id, '_joomsport_match_date', ($fdata['m_date'][$intA])); 
                    }
                    if(isset($fdata['m_time'][$intA]) && (!isset($allmeta["_joomsport_match_time"][0]) || $allmeta["_joomsport_match_time"][0] != ($fdata['m_time'][$intA]))){
                        update_post_meta($post_id, '_joomsport_match_time', ($fdata['m_time'][$intA])); 
                    }
                    if(isset($fdata['venue_id'][$intA]) && (!isset($allmeta["_joomsport_match_venue"][0]) || $allmeta["_joomsport_match_venue"][0] != intval($fdata['venue_id'][$intA]))){
        ...
        $metasMD["is_playoff"] = isset($fdata["is_playoff"])?intval($fdata["is_playoff"]):0;
